function [imfinal,depthdata]=good_depth_example()
    ...
    rgb_sensor.set_option(optionType, 625);  % Set exposure to 6000 (adapt as needed)
    ...
    data = color.get_data();
    im = permute(reshape(data',[3,color.get_width(),color.get_height()]),[3 2 1]);
    % figure;
    % imfinal = im;
    % imshow(im);
    % title(sprintf("Cojsdjlor RGB frame from %s", name));
    % Alignment
